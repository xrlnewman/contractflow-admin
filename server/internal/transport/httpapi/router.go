...
import (
	...
	"github.com/xrlnewman/contractflow-admin/server/internal/app/dispatch"
	"github.com/xrlnewman/contractflow-admin/server/internal/app/inventory"
	orderapp "github.com/xrlnewman/contractflow-admin/server/internal/app/order"
	"github.com/xrlnewman/contractflow-admin/server/internal/config"
	"github.com/xrlnewman/contractflow-admin/server/internal/domain"
	platformauth "github.com/xrlnewman/contractflow-admin/server/internal/platform/auth"
	"github.com/xrlnewman/contractflow-admin/server/internal/platform/cache"
...
func NewRouterWithDeps(cfg config.Config, st *store.MemoryStore, deps Dependencies) *gin.Engine {
	...
	// 合同领域别名：审核与签署归档复用统一幂等状态服务。
	protected.GET("/contracts", s.stockPurchases)
	protected.POST("/contracts/:id/approve", s.stockReceive)
	protected.GET("/contract-signatures", s.stockSales)
	protected.POST("/contract-signatures/:id/complete", s.stockShip)
	protected.GET("/contract-events", s.stockMovements)
